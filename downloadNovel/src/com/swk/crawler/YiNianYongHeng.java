...
public class YiNianYongHeng {
	...
	public static void download() throws IOException{
		...
		File file = new File("src/com/swk/novel/ynyh.txt");//小说本地目录
		File errorFile = new File("src/com/swk/novel/error.txt");//错误章节目录
		Writer w = new FileWriter(file);
		Writer w2 = new FileWriter(errorFile);
		
		//获取章节列表
		List<HashMap<String, String>> chapterList = YiNianYongHeng.getChapterList();
...
//		}
		...
		for(int i = chapterList.size() - 15; i < chapterList.size(); i++){
			chapterUrl = new StringBuilder(chapterList.get(i).get("chapterHref"));
			chapterText = new StringBuilder(YiNianYongHeng.getChapter(chapterUrl.toString()));
...
//		List<HashMap<String, String>> l = YiNianYongHeng.getChapterList();
...
//		String s = YiNianYongHeng.getChapter(url);
//		System.out.println(s);
		
		YiNianYongHeng.download();
	}
